...
def test_generate_mypy_matches() -> None:
    ...
    assert len(proto_files) == 95  # Just a sanity check that all the files show up

    pyi_files = glob.glob("test/generated/**/*.pyi", recursive=True)
    assert len(pyi_files) == 97  # Should be higher - because grpc files generate extra pyis
