...
class PkgWriter(object):
    ...
    def write_grpc_type_vars(self, service: d.ServiceDescriptorProto) -> None:
        ...
        for _, method in methods:
            ...
            if self.grpc_type != GRPCType.BOTH:
                type_var = _build_typevar_name(service.name, method.name)
                callable_type = self._callable_type(method, is_async=self.grpc_type == GRPCType.ASYNC)
                wl("{}: {} = {}[", type_var, self._import("typing_extensions", "TypeAlias"), callable_type)
                with self._indent():
                    wl("{},", self._input_type(method))
                    wl("{},", self._output_type(method))
                wl("]")

            # Write both sync and async as a TypeVar
            else:
        ...
        for i, service in enumerate(services):
            ...
            with self._indent():
                ...
                if self.grpc_type == GRPCType.BOTH:
                    ...
                    self.write_self_types(service, is_async=False)
                    wl(
                        "], channel: {}) -> None: ...",
                    ...
                    wl("@{}", self._import("typing", "overload"))
                    wl("def __init__(self: {}[", class_name)
                    self.write_self_types(service, is_async=True)
                    wl(
                        "], channel: {}) -> None: ...",
            ...
            if self.grpc_type == GRPCType.BOTH and self.grpc_type.supports_async:
                # Write AsyncStub alias (only when Stub is generic)
